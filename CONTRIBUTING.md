...
When you run `yarn install` or `npm install` inside of `~/code/node_modules/dat`, dat will get its own copy of `hyperdrive` (one if its dependencies) inside `~/code/node_modules/dat/node_modules` from the npm registry. However, if you encounter a bug in hyperdrive that you need to fix, but you want to test your fix in dat, you want dat to use your git copy of hyperdrive at `~/code/node_modules/hyperdrive` and not the npm registry copy of hyperdrive at `~/code/node_modules/dat/node_modules/hyperdrive`.

How do you get dat to use the git copy of hyperdrive? Just delete the npm registry copy!
...
If you want to switch back to npm registry copy, just run `yarn install` or `npm install`  inside `~/code/node_modules/dat/` and any missing modules will be downloaded into `~/code/node_modules/dat/node_modules` without touching anything in `~/code/node_modules`.
...
- Never make any meaningful edits to code inside an "npm/yarn-managed" node_modules folder (such as `~/code/node_modules/dat/node_modules`), because any changes inside those folders could inadvertently be deleted when installing an updated copy of a module. This has happened to me many times, so I just always use my git copy and delete the npm registry copy (as described above) to make edits to a module.
- You should never need to run any npm/yarn commands in terminal when at your "manually managed"" node_modules folder at `~/code/node_modules`. Never running npm/yarn commands at that folder also your git copies of the modules from getting erased.
